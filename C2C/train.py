...
def train_model(model, criterion_dic, optimizer, df, data_transforms, alpha=1., beta=0.01, gamma=0.01,
                num_cluster=8, num_img_per_cluster=8, num_epochs=25, fpath='checkpoint.pt', topk=False, wandb_monitor=False):
    """ Function for training
    """
    if wandb_monitor:
        wandb.init(project="C2C")
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", 'val']:
            ...
            # Iterate over data.
            for i, (inputs, labels, inputs_cluster) in enumerate(tqdm(dataloaders[phase], desc="Iterating data", total=len(dataloaders[phase]))):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # print("outputs_patch --> ", outputs_patch)
                    # print("patch_labels -->", patch_labels)
            ...
            if wandb_monitor:
                wandb.log({"Patch Loss": epoch_loss_patch, "Loss WSI":epoch_loss_wsi, "Loss KLD":epoch_loss_kld, "Training Accuracy":epoch_acc})

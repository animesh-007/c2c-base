...
class WSIClassifier(nn.Module):
    ...
    def forward(self, x): # torch.Size([1, 8, 3, 512, 512])
        x = x.view(-1, x.shape[-3], x.shape[-2], x.shape[-1]) # torch.Size([8, 3, 512, 512])
        x = self.resnet_head(x) # torch.Size([8, 512, 1, 1])
        x = x.view(x.size(0), -1) # torch.Size([8, 512])
        x = self.resnet_tail(x)   # torch.Size([8, 64])
        xp = self.patch_classifier(x) # torch.Size([8, 6])
        
        A_unnorm = self.attention(x) # torch.Size([8, 1])
        A = torch.transpose(A_unnorm, 1, 0) # torch.Size([1, 8])
        A = F.softmax(A, dim=1)
        
        M = torch.mm(A, x) # torch.Size([1, 64])
        Y_prob = self.classifier(M)  # torch.Size([1, 6])    
...
class EncAttn(nn.Module):
    ...
    def forward(self, x): # torch.Size([17, 3, 512, 512])
        x = self.resnet_head(x) # torch.Size([17, 512, 1, 1])
        x = x.view(x.size(0), -1) # torch.Size([17, 512])
        x = self.resnet_tail(x) # torch.Size([17, 64])
        attn = self.attention(x) # torch.Size([17, 1])
